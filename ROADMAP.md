...
- 用户级覆盖：`~/.javacli/prompts/base.md` 可整体替换内置 base.md；`~/.javacli/prompts/modes/agent.md` 可覆盖特定模式
...
*已完成第 16 期 TUI 产品化（含 16.1 形态修正：默认切换为 inline 流式 TUI，Lanterna 全屏 TUI 通过 `JAVACLI_RENDERER=lanterna` 保留）、第 17 期 LSP 诊断注入 MVP，并开始落地第 18 期 Git Side-History 快照与回滚 MVP。后续第 19–20 期依次做 Prompt 分层、异步后台任务，第 21 期（原 17 期）多模态 vision 排在安全网与架构就绪之后。*
