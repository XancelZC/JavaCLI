...
import com.javacli.llm.LlmTraceLogger;
...
import com.javacli.memory.ExplicitMemoryHints;
import com.javacli.memory.MemoryManager;
import com.javacli.prompt.PromptAssembler;
import com.javacli.prompt.PromptContext;
import com.javacli.prompt.PromptMode;
import com.javacli.render.PlainRenderer;
...
import com.javacli.skill.SkillIndexFormatter;
import com.javacli.skill.SkillRegistry;
import com.javacli.util.AnsiStyle;
import com.javacli.tool.ToolRegistry;
import com.javacli.tool.ToolRegistry.ToolExecutionResult;
import com.javacli.tool.ToolRegistry.ToolInvocation;
import com.javacli.util.TerminalMarkdownRenderer;
import com.javacli.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String getContextStatus() {
        com.javacli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.javacli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.javacli.memory.MemoryEntry.estimateTokens(
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.javacli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.javacli.memory.MemoryEntry.estimateTokens(
