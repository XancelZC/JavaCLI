 ...
 * 1. 系统属性：{@code javacli.react.token.budget} / {@code javacli.react.stagnation.window} /
 *    {@code javacli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Djavacli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Djavacli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("javacli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("javacli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("javacli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
