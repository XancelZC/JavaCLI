...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("javacli.snapshot.enabled", "JAVACLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("javacli.snapshot.dir", "JAVACLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".javacli", "snapshots").toString()));
        int max = readInt("javacli.snapshot.max", "JAVACLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("javacli.snapshot.excludes", "JAVACLI_SNAPSHOT_EXCLUDES", ""));
