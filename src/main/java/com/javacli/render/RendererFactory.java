 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Djavacli.renderer} > {@code JAVACLI_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code JAVACLI_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        String prop = System.getProperty("javacli.renderer");
        if (prop != null && !prop.isBlank()) {
            return parse(prop);
        }
        String env = System.getenv("JAVACLI_RENDERER");
        if (env != null && !env.isBlank()) {
            return parse(env);
        }
        // 兼容旧 JAVACLI_TUI=true → lanterna
        String legacyTui = System.getenv("JAVACLI_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ JAVACLI_TUI=true 已废弃，请改用 JAVACLI_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 JAVACLI_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
