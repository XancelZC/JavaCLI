...
```bash
ls ~/.javacli/skills-cache/web-access/references/site-patterns/
```
...
**写回机制**：操作中如果发现新站点的陷阱或有效模式，主动写到 `~/.javacli/skills/web-access/references/site-patterns/<domain>.md`（用户级目录，不要写 jar 内置缓存），格式参考已有文件。
