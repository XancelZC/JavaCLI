...
JavaCLI 第 13 期接入 Google 官方 `chrome-devtools-mcp@latest`（28 个工具），第 14 期加 `/browser connect` 切 shared 模式。
...
- `new_page` — 开新 tab（JavaCLI 记录为 agent-opened，可被 close_page 关）
- `select_page` — 切换激活 tab
- `close_page` — 关 tab（**只能关 JavaCLI 自己 new_page 出来的**）
- `list_pages` — 列出所有 tab
...
1. **截图无意义**：`take_screenshot` 的 image content LLM 看不到，JavaCLI 现版本不直传给模型。一定要看页面布局时用，并且自己向用户描述。
