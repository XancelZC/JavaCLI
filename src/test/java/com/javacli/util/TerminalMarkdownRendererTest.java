...
class TerminalMarkdownRendererTest {
    static {
        System.setProperty("javacli.render.color", "false");
    }
    ...
    void fallsBackToKeyValueLayoutForLongTwoColumnTable() {
        String markdown = """
                | 目录名 | 说明 |
                | --- | --- |
                | src/main/java/com/javacli | 这里存放 JavaCLI 的主要 Java 源码实现与相关模块 |
                """;

        String rendered = TerminalMarkdownRenderer.render(markdown);

        assertTrue(rendered.contains("目录名 / 说明"));
        assertTrue(rendered.contains("- src/main/java/com/javacli"));
        assertTrue(rendered.contains("这里存放 JavaCLI 的主要 Java 源码实现与相关模块"));
    }
}
