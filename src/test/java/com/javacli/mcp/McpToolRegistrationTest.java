...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("javacli.audit.dir");
        System.setProperty("javacli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("javacli.audit.dir");
            } else {
                System.setProperty("javacli.audit.dir", previous);
            }
        }
    }
