...
class LongTermMemoryTest {
    ...
    void shouldStoreAndRetrieve() {
        MemoryEntry entry = new MemoryEntry("fact-1", "项目使用Java 21", MemoryEntry.MemoryType.FACT, null, 10);
        memory.store(entry);

        assertTrue(memory.retrieve("fact-1").isPresent());
        assertEquals("项目使用Java 21", memory.retrieve("fact-1").get().getContent());
    }
    ...
    void shouldFilterProjectScopedMemories() {
        ...
        memory.store(new MemoryEntry("project-a", "项目A使用 Java 21", MemoryEntry.MemoryType.FACT,
